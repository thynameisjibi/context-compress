...
use context_compress_core::{
    AbstractiveCompressor, ExtractiveCompressor, HybridCompressor, TokenCounter,
};
...
fn benchmark_hybrid_compression(c: &mut Criterion) {
    let compressor = HybridCompressor::default().with_abstractive(AbstractiveCompressor::default());
